...
public abstract class OpGrab extends Thread implements PtGuiStateCallback {
	...
	public void run() {
		...
		try {
			...
			image = GrabImage(mbyGrabType);
			iWidth = (mConn.info().imageWidth);
			...
			case PtConstants.PT_GRAB_TYPE_508_508_8_SCAN508_508_8:
				...
				onCallbackImage(CreateBitmap(image, iWidth));
				break;
			default:
	...
	private void ModifyConfig() throws PtException {
		/*
		 * try { final short SESSION_CFG_VERSION = 5; PtSessionCfgV5 sessionCfg
		 * = (PtSessionCfgV5) mConn.getSessionCfgEx(SESSION_CFG_VERSION);
		 * 
		 * mConn.setSessionCfgEx(SESSION_CFG_VERSION, sessionCfg); } catch
		 * (PtException e) { onDisplayMessage("Unable to set session cfg - " +
	 ...
	 * Get finger image
	 */
	abstract protected void onCallbackImage(Bitmap mBitmap);
