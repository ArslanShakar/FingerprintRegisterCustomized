...
public abstract class OpEnroll extends Thread implements PtGuiStateCallback {
	...
	public void run() {
		try {
			...
			// If yes and template corresponds to finger ID, remove it.
			...
			// This code show how to convert templates, it makes 2 conversions
			// (to ISO template and back)
			// and verifies that the result match with the original template
			// Note: PtConvertTemplateEx is supported only by TCD50 V3 (TCD50
			// with area sensor) and TCD51
			if (false) {
				...
				// Convert ISO template back to alpha template, get raw template
				// (without header)
				byte[] aAlphaRawTemplate = mConn.convertTemplateEx(PtConstants.PT_TEMPLATE_TYPE_ISO_FMR,
						PtConstants.PT_TEMPLATE_ENVELOPE_NONE, mIsoRawTemplate, PtConstants.PT_TEMPLATE_TYPE_ALPHA,
						PtConstants.PT_TEMPLATE_ENVELOPE_NONE, null, 0);
				Log.d("jokey", "alpha(without header)--->template"+DataUtils.toHexString(aAlphaRawTemplate));
				...
				// add raw alpha template data to this header, round up size to
				// 4 and add empty payload (another 4 zero bytes)
				...
				Log.d("jokey", "alpha(with header)--->template"+DataUtils.toHexString(aAlphaInputBir.bir.data));	
				// Match the resulting template against the old template from
				// PtEnroll, should match
				if (mConn.verify(0, 0, false, aAlphaInputBir, null, null, null, null,
	...
	/**
	 * Test, if finger already isn't enrolled in device. If yes and template
	 * corresponds to finger ID, remove it. Otherwise report an error.
	 * 
				...
				for (int i = 0; i < fingerList.length; i++) {
					...
					if ((fingerData != null) && (fingerData.length >= 1)) {
						...
						if (fingerId == mFingerId) {
							...
							// Verify, if template doesn't match the enrolled
							// one (last good template)
							if (mConn.verifyMatch(null, null, null, null, comparedBir, null, null, null,
									null) == true) {
